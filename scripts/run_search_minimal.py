...
from src.core.lm import VLLMRunner
from src.core.trace.writer import TraceLogger
from src.core.eval_minimal import extract_gt_from_answer_field
from src.core.prm_remote import RemotePRMScorer
from src.search.mcts_minimal import MCTSMinimal


def infer_dataset_name(sample, dataset_path: str, dataset_name_arg: str = "auto") -> str:
    """
    数据集识别优先级：
    1. 显式命令行参数 --dataset_name（若不是 auto）
    2. sample["dataset"]
    3. 文件名启发式判断
    """
...
def main():
    ...
    ap.add_argument("--runner", type=str, default="vllm", choices=["vllm"])
    ap.add_argument("--model_ckpt", type=str, required=True)
    ap.add_argument("--tp", type=int, default=1)

    ap.add_argument("--gpu_memory_utilization", type=float, default=0.20)
    ap.add_argument("--vllm_max_num_seqs", type=int, default=16)
    ap.add_argument("--vllm_max_num_batched_tokens", type=int, default=None)
    ap.add_argument("--num_gpu_blocks_override", type=int, default=None)
    ap.add_argument("--disable_prefix_caching", action="store_true")
    ap.add_argument("--vllm_dtype", type=str, default="auto")
    ap.add_argument("--vllm_seed", type=int, default=0)
    ap.add_argument("--vllm_logprobs", type=int, default=1)
    ap.add_argument("--max_model_len", type=int, default=1024)

    ap.add_argument("--vllm_enable_metrics", action="store_true")
    ap.add_argument("--vllm_enable_kv_cache_metrics", action="store_true")
    ap.add_argument("--vllm_kv_cache_metrics_sample", type=float, default=1.0)
    ap.add_argument("--kv_cache_memory_bytes", type=str, default=None)
    ...
    # batch-first: 至少保证 vLLM 可同时容纳一整个 sibling batch
    min_needed_seqs = max(args.branch_factor, args.simulation_branch_factor)
    if args.vllm_max_num_seqs < min_needed_seqs:
        print(
            f"[run_search_minimal] bump vllm_max_num_seqs: "
            f"{args.vllm_max_num_seqs} -> {min_needed_seqs}"
        )
        args.vllm_max_num_seqs = min_needed_seqs
    ...
    lm = VLLMRunner(
        ...
        enable_metrics=args.vllm_enable_metrics,
        kv_cache_metrics=args.vllm_enable_kv_cache_metrics,
        kv_cache_metrics_sample=args.vllm_kv_cache_metrics_sample,
    ...
    for state in sample_states:
        ...
        sample_info = {
            ...
            "search_mode": "batch_vllm_expand_and_simulate",
