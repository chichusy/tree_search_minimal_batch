...
OUTDIR=${OUTROOT}/debug_expand_batch_sim_${SLURM_JOB_ID}
...
CUDA_VISIBLE_DEVICES=1 "${PRM_PY}" -m no_vllm.scripts.prm_server \
  --host 127.0.0.1 \
  --port 18080 \
...
CUDA_VISIBLE_DEVICES=0 "${MAIN_PY}" -m no_vllm.scripts.run_search_minimal \
  --runner hf \
